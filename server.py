...
class client:
    ...
    def get_data(self):
        # self.conn, self.addr = self.sock.accept()
        current_data = self.conn.recv(1024).decode('utf-8')
        print("Received: ", current_data)
        return current_data

    def get_data_raw(self):
        current_data = self.conn.recv(1024)
    ...
    def send_data(self, message):
        ...
        # if self.conn == None:
            # raise Exception("No connection has been established yet")
            # return
...
def send_pieces_to_opponent(game_client, pieces):
    ...
    print(piece_count)
    game_client.send_data(str(piece_count))
    ...
    for each_team in pieces:
        for each_piece in each_team:
            ...
            # print(each_piece)
            game_client.send_data_raw(current_piece)
            sleep(0.0001)

    # Now check if client is ready


def get_pieces_from_opponent(game_client):
    # Get the amount of pieces from opponent
    piece_count = game_client.get_data()
...
def send_gamestate_to_opponent(game_client, gamestate):

    # If no one has won
    if gamestate == False:
        game_client.send_data("No winner")

    elif gamestate == "white":
        game_client.send_data("White won!")

    else:
        game_client.send_data("Black won!")

    return


def get_gamestate_from_opponent(game_client):
    data = game_client.get_data()
...
def main():
    ...
    game_client = client(host, port)
    game_client.bind()

    game_client.accept()
    game_client.send_data('Thanks for playing Checkers! Created by Tyler Aubin, Jordan Allen')
    game_client.send_data('The rules are standard, with minor changes. There is no double jumping, it ruins friendships. Oh, and while you are at it, select a color to play as. White starts the game!')
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if piece is not None:
                        ...
                        # If some error in move (Invalid square clicked, jumping over own piece, etc)
                        if not piece:
                            # Go back to square 1
                            piece = None
                            continue

                        # Otherwise, move is valid, and should be accounted for
                        # print(f'Old Piece: {old_piece} --- New Piece: {piece}')
                        ...
                        send_gamestate_to_opponent(game_client, winner)
                        # If somebody won the game
                        if winner is not False:
                            print("Sending win to other side")
                            game_client.shutdown()
                            exit(0)
                        send_pieces_to_opponent(game_client, pieces)
                        ...
                        if get_piece(mouse_position, pieces, LIGHT) is None:
                            continue
                        else:
                            piece, piece_index, piece_team = get_piece(mouse_position, pieces, LIGHT)
                            potential_moves = get_potential_moves(piece, pieces)
        else:
            game_state = get_gamestate_from_opponent(game_client)

            if game_state != "No winner":
                print(game_state)
                # game_client.send_data('Closing Connection')
                game_client.shutdown()
                exit(0)


            pieces = get_pieces_from_opponent(game_client)
